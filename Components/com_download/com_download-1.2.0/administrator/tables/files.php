...
class DownloadTablefiles extends JTable {
    ...
    public function bind($array, $ignore = '') {
			...
			$array['state'] = 0;
				...
				if(!empty($files['file'])){
					...
					if($fileError > 0 && $fileError != 4) {
						...
						$uploadPath = JPATH_ADMINISTRATOR . '/components/com_download/images/Downloads/' . $filename;
		...
		//Support for multiple SQL field: whatbanner
			if(isset($array['whatbanner'])){
				if(is_array($array['whatbanner'])){
					$array['whatbanner'] = implode(',',$array['whatbanner']);
				}
				else if(strrpos($array['whatbanner'], ',') != false){
					$array['whatbanner'] = explode(',',$array['whatbanner']);
				}
				else if(empty($array['whatbanner'])) {
					$array['whatbanner'] = '';
				}
			}
	...
	jimport('joomla.filesystem.file');
	$result = JFile::delete(JPATH_ADMINISTRATOR . '/components/com_download/images/Downloads/' . $this->file);
        }
        return $result;
    }
