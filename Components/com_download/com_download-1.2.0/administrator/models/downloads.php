...
class DownloadModelDownloads extends JModelList {
    ...
    public function getItems() {
		...
		foreach ($items as $oneItem) {
			...
			if (isset($oneItem->whatbanner)) {
				$values = explode(',', $oneItem->whatbanner);

				$textValue = array();
				foreach ($values as $value){
					$db = JFactory::getDbo();
					$query = "SELECT alias FROM #__banners WHERE alias LIKE '" . $value . "'";
					$db->setQuery($query);
			...
			$oneItem->whatbanner = !empty($textValue) ? implode(', ', $textValue) : $oneItem->whatbanner;
