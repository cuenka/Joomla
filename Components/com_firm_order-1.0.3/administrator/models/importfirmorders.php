...
/**
 * Methods supporting a list of Firm_order records.
 */
class Firm_orderModelImportfirmorders extends JModelList {
    ...
    protected function populateState($ordering = null, $direction = null) {
        ...
        // Load the parameters.
        $params = JComponentHelper::getParams('com_firm_order');
    ...
    protected function getListQuery() {
		...
		if ($ReadytoImport=='import') {
			Firm_orderModelImportfirmorders::getFile($ReadytoImport);
		}elseif($File!=null){
			$Error = Firm_orderModelImportfirmorders::checkFile($File);
			if ($Error)	JError::raiseWarning( $File['error'], $Error);
				else Firm_orderModelImportfirmorders::getFile($ReadytoImport);
		}else {
			Firm_orderModelImportfirmorders::getRows();
		}
		...
		if (!empty($message)) {
		 ...
		 $data = Firm_orderModelImportfirmorders::getFile($File); 
		}
				*/
		/*
		echo $foo; */
	...
	if (($handle = fopen($dest, 'r')) !== FALSE)
	    {
	        while (($row = fgetcsv($handle, 1000)) !== FALSE)
	        {
	            if(!$header)
	            	...
	            	if ($ReadytoImport=='import') 	Firm_orderModelImportfirmorders::InsertRow($header,$row);
	                $data[] = array_combine($header, $row);
	        }
	        fclose($handle);
	    }
	 }   
	 if ($ReadytoImport=='import') Firm_orderModelImportfirmorders::DeleteFile($dest);
return $data;
	}
	...
	protected function InsertRow($header,$row) {
		...
		$query
		    ->insert($db->quoteName('#__com_firm_order'))
