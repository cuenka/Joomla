 ...
 */
class Industry_data_tableModelExportimportdatas extends JModelList {
    ...
    protected function getListQuery() {
    	...
    	if ($ReadytoImport=='import') {
    		Industry_data_tableModelExportimportdatas::getFile($ReadytoImport);
    	}elseif($File!=null){
    		$Error = Industry_data_tableModelExportimportdatas::checkFile($File);
    		if ($Error)	JError::raiseWarning( $File['error'], $Error);
    			else Industry_data_tableModelExportimportdatas::getFile($ReadytoImport);
    	}else {
    		Industry_data_tableModelExportimportdatas::getRows();
    	}
    	...
    	if (!empty($message)) {
    	 ...
    	 $data = Industry_data_tableModelExportimportdatas::getFile($File); 
    	}
    			*/
    	/*
    	echo $foo; */
		...
		$query->from('`#__industry_data_table_aircraft_deals`');
		$db->setQuery($query);  
		return $query; 
	}
	...
	if (($handle = fopen($dest, 'r')) !== FALSE)
	    {
	        while (($row = fgetcsv($handle, 1000)) !== FALSE)
	        {
	            if(!$header)
	            	...
	            	if ($ReadytoImport=='import') 	Industry_data_tableModelExportimportdatas::InsertRow($header,$row);
	                $data[] = array_combine($header, $row);
	        }
	        fclose($handle);
	    }
	 }   
	 if ($ReadytoImport=='import') Industry_data_tableModelExportimportdatas::DeleteFile($dest);
return $data;
	}
	...
	protected function InsertRow($header,$row) {
		...
		$query
		    ->insert($db->quoteName('#__industry_data_table_aircraft_deals'))
