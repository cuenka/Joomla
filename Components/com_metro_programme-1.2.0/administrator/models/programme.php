 ...
 */
class Metro_programmeModelProgramme extends JModelAdmin
{
	/**
	 * @var		string	The prefix to use with controller messages.
	 * @since	1.6
	 */
	protected $text_prefix = 'COM_METRO_PROGRAMME';
	 ...
	 * @since	1.6
	 */
	public function getTable($type = 'Programme', $prefix = 'Metro_programmeTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true)
	{
		...
		// Get the form.
		$form = $this->loadForm('com_metro_programme.programme', 'programme', array('control' => 'jform', 'load_data' => $loadData));
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_metro_programme.edit.programme.data', array());
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id)) {
			...
			if (@$table->ordering === '') {
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__metro_programme');
				$max = $db->loadResult();
